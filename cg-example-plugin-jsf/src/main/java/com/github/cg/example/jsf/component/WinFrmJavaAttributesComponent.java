	...
	public String render() {
		...
		for (AttributeOneToMany atributo : getTargetContext().getEntity().getAttributesOneToMany()) {
			if (AttributeFormType.INTERNAL.equals(atributo.getFormType())) {
				println(sb, "\tprivate FrmAssociationOneToMany<WinFrm{0}, {0}, {1}> association{2};", getTargetContext().getEntity().getName(), atributo.getAssociationClassSimpleName(), atributo.getNameFuc());
			}
			else {
				println(sb, "\t@Inject");
				println(sb, "\t@New");
				println(sb, "\tprivate WinFrm{0} winFrm{0};", atributo.getAssociationClassSimpleName());	
				println(sb, "\tprivate WinFrmAssociationOneToMany<WinFrm{0}, WinFrm{1}, {0}, {1}> association{2};", getTargetContext().getEntity().getName(), atributo.getAssociationClassSimpleName(), atributo.getNameFuc());
			}
		}
